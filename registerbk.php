...
// Validate username
if (!preg_match('/^(?=.*[A-Za-z])(?=.*\d)[A-Za-z\d]{5,}$/', $username)) {
...
$check_sql = "SELECT * FROM userdetails WHERE username = '$username' OR email = '$email' OR password = '$password'";
$result = $conn->query($check_sql);

if ($result->num_rows > 0) {
    echo "<script type='text/javascript'>alert('A user with the same username, email, password already exists. Please use different details.');window.history.back();</script>";
    exit;
}
