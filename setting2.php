...
// Fetch all rows from the `curriculum` table
$sql = "SELECT * FROM curriculam";
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['delete_id'])) {
    ...
    $delete_sql = "DELETE FROM curriculam WHERE id = ?";
    $stmt = $conn->prepare($delete_sql);
    $stmt->bind_param("i", $delete_id);

    if ($stmt->execute()) {
        echo "<script>alert('Entry deleted successfully.'); window.location.href = 'setting2.php';</script>";
    } else {
...
<body>
    <div class="container">
        ...
        <tbody>
                    ...
                    <tr>
                        ...
                        <td>
                            <div class="action-buttons">
                                <a class="btn btn-edit" href="edit_curriculam.php?id=<?php echo $row['id']; ?>">Edit</a>
            ...
            <?php else: ?>
                <tr>
                    <td colspan="6">No curriculum entries found</td>
                </tr>
            <?php endif; ?>
        </tbody>
        </table>
